...
public class TileDriveT2 extends TileDrive implements IChestOrDrive, IPriorityHost {

    private final AppEngCellInventory inv = new AppEngCellInventory(this, 20);
    private final ICellHandler[] handlersBySlot = new ICellHandler[20];
    private final DriveWatcher<IAEItemStack>[] invBySlot = new DriveWatcher[20];
    ...
    private void updateState() {
        if (!this.isCached) {
            ...
            storageChannels.forEach(channel -> this.inventoryHandlers.put(channel, new ArrayList<>(20)));
    ...
    public ItemStack getItemStackRepresentation() {
        return AEApi.instance().definitions().blocks().driveT2().maybeStack(1).orElse(ItemStack.EMPTY);
    }

    @Override
    public GuiBridge getGuiBridge() {
        return GuiBridge.GUI_DRIVE_T2;
    }
}
