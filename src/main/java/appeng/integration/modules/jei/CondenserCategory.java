...
class CondenserCategory implements IRecipeCategory<CondenserOutputWrapper> {
    ...
    private List<ItemStack> getViableStorageComponents(CondenserOutputWrapper recipeWrapper) {
        ...
        materials.cell1024kPart().maybeStack(1).ifPresent(itemStack -> this.addViableComponent(condenserOutput, viableComponents, itemStack));
        materials.cell4096kPart().maybeStack(1).ifPresent(itemStack -> this.addViableComponent(condenserOutput, viableComponents, itemStack));
        materials.cell16384kPart().maybeStack(1).ifPresent(itemStack -> this.addViableComponent(condenserOutput, viableComponents, itemStack));
